...
export type DuoDirectoryTab = 'file' | 'git' | 'conversation'
export type PersistedDuoFileDiff = { scope: GitChangeScope; mode: 'split' | 'unified' }
export type PersistedDuoTab = { id: string } & (
  | { kind: 'file'; path: string; workspacePath: string | null; viewMode: WorkspaceFileViewMode; gitDiff?: PersistedDuoFileDiff }
...
export type PersistedDuoPane = {
  directoryOpen: boolean
  directoryTab: DuoDirectoryTab
  activeTabId: string
  tabs: PersistedDuoTab[]
}
export type PersistedDuoLayout = {
  version: 1
  focusedPane: 'left' | 'right'
  ratio: number
  panes: Record<'left' | 'right', PersistedDuoPane>
}

// Project IDs, rather than names or the last selected CWD, own workspace state.
export type PersistedDuoProjects = {
  version: 1
  layouts: Record<string, PersistedDuoLayout>
}

export function normalizeDuoProjects(value: unknown): PersistedDuoProjects | undefined {
  ...
  return { version: 1, layouts: Object.fromEntries(Object.entries(record(state.layouts)).flatMap(([id, value]) => {
    const layout = normalizeDuoLayout(value)
...
function normalizeTab(value: unknown): PersistedDuoTab | null {
  ...
  if (tab.kind === 'file' && text(tab.path)) {
    ...
    const gitDiff: PersistedDuoFileDiff | undefined = (diff.scope === 'staged' || diff.scope === 'unstaged')
...
export function normalizeDuoLayout(value: unknown): PersistedDuoLayout | undefined {
  const state = record(value)
  const panes = record(state.panes)
  if (state.version !== 1 || !panes.left || !panes.right) return undefined
  const conversations = new Set<string>()
  const normalizePane = (value: unknown): PersistedDuoPane => {
