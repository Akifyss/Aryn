import { PrioritizedTaskLane, type TaskPriority } from './prioritized-task-lane'
...
type RuntimeEntry<TRuntime> = {
  ...
  generation: number
  key: string
  lifecycleLane: PrioritizedTaskLane
  startingGeneration: number | null
}

export type SessionRuntimeEventResult =
  | { status: 'applied' }
  | { status: 'dropped' }
  | { error: Error, status: 'failed' }

export type SessionRuntimeEventReceipt = {
  readonly generation: number
  readonly key: string
  readonly settled: Promise<SessionRuntimeEventResult>
}
...
export class SessionRuntimeCoordinator<TRuntime> {
  ...
  async ensure(
    ...
    if (entry.lifecycleLane.pending === 0 && entry.current) return entry.current
  ...
  private enqueueEvent(
    ...
    let settle!: (result: SessionRuntimeEventResult) => void
    const receipt: SessionRuntimeEventReceipt = {
      generation: lease.generation,
      key: lease.key,
      settled: new Promise<SessionRuntimeEventResult>((resolve) => {
        settle = resolve
      }),
    }
    if (!this.isLeaseCurrent(entry, lease)) {
      settle({ status: 'dropped' })
      return receipt
    }
    ...
    entry.eventTail = execution
      .catch((error) => {
        ...
        settle({ error: normalizedError, status: 'failed' })
  ...
  private getOrCreateEntry(key: string) {
    ...
    const entry: RuntimeEntry<TRuntime> = {
      ...
      generation: 0,
      key,
      lifecycleLane: new PrioritizedTaskLane(),
