...
import { useWindowChrome } from '@/features/layout/hooks/use-window-chrome'
import { useAppAppearanceController } from '@/features/appearance/hooks/use-app-appearance-controller'
import { openWorkbenchProjectSession, useWorkbenchStore } from '@/features/workbench/workbench-state'
import { useProjectWorkspaceSync } from '@/features/workbench/use-project-workspace-sync'
import { flushWorkbenchPersistence } from '@/features/workbench/workbench-persistence'
import { WorkspaceShell, startWorkbenchConversation, type WorkspaceShellHandle } from '@/features/workbench/workspace-shell'
...
function App() {
  ...
  const workbenchRef = useRef<WorkspaceShellHandle>(null)
  const workbenchInitialized = useWorkbenchStore((state) => state.initialized)
  const captureDocumentTarget = useCallback(() => captureWorkbenchDocumentTarget(), [])
  const workspaceEditorSurface = useWorkspaceTabViewState({
    activeTabId,
    openTabs,
  })
  const {
  ...
  }, [currentPath, ensureFullyLoadedWorkspaceTree])
  ...
  const windowChrome = useWindowChrome(platform)
  ...
  const isGlobalProjectMenuOpen = isProjectMenuOpen
  const {
    closeCommandPalette,
    isAppModalLayerOpen,
  ...
  const handleRequestWindowClose = useAppWindowClose({
    confirmDiscardDirtyTabs,
    beforeClose: flushWorkbenchPersistence,
  })
  ...
  useAppKeyboardShortcuts({
    activeTabId: 'workbench-active-tab',
    closeActiveTab: () => workbenchRef.current?.closeActiveTab(),
    cycleTabs: (direction) => workbenchRef.current?.cycleTabs(direction),
    isShortcutBlockingLayerOpen,
    onSaveActiveTab: () => {
      const workbench = useWorkbenchStore.getState()
      const pane = workbench.panes[workbench.focusedPane]
      if (pane.tabs.some((tab) => tab.kind === 'document' && tab.id === pane.activeTabId)) return handleSaveActiveTab()
    },
    onStartContextualConversation: () => startWorkbenchConversation(activeProject, () => openProjectMenu('editor-switch')),
    onToggleCommandPalette: toggleCommandPalette,
    platform,
  })

  useProjectWorkspaceSync({ initialized: workbenchInitialized, bootstrapReady,
  ...
  const handleOpenSession = useCallback((sessionPath: string, sessionLabel: string) => {
    ...
    openWorkbenchProjectSession(useWorkbenchStore.getState().focusedPane, activeProject, {
      sessionPath,
  ...
  return (
    <>
        <WorkspaceShell
          ...
          chromeVars={windowChrome.shellChromeVars}
          platform={windowChrome.shellPlatform}
          isFullScreen={windowChrome.isWindowFullScreen}
