...
let configuredMermaidRuntimeSource: string | null = null;
...
export function setMermaidRuntimeSource(source: string | null | undefined): string | null {
  const previousSource = configuredMermaidRuntimeSource;
  configuredMermaidRuntimeSource = typeof source === 'string' && source.trim() ? source.trim() : null;
  return previousSource;
}

function getMermaidRuntimeSource() {
  return configuredMermaidRuntimeSource
    ?? document.body?.dataset?.meoMermaidSrc
    ?? '';
}
...
export class MermaidDiagramWidget extends WidgetType {
  ...
  createFullscreenControls(svgContainer) {
    ...
    exitBtn.appendChild(createElement(X));
