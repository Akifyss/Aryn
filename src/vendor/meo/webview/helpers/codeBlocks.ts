  ...
  token: (stream: any, state: CSharpState) => {
    ...
    if (stream.match(/^@?[A-Za-z_]\w*/)) {
      ...
      if (state.expectTypeName) {
        state.expectTypeName = false;
        // new Foo() â€?type followed by '(' is still a type (constructor)
        return "typeName";
      }
      ...
      // PascalCase followed by '<' â€?generic type: List<T>
    ...
    // Colon â€?expect type name after ':' (inheritance, type constraints)
    if (stream.match(":")) {
      state.expectTypeName = true;
      return "punctuation";
    }

    // Angle brackets â€?'<' expects type arg, '>' does not
    if (stream.match("<")) {
      state.expectTypeName = true;
      return "angleBracket";
    }
