...
// Keep layout.duo as a legacy archive. Only its first migration populates the
// project map, so closing every tab never makes an old layout reappear.
export function migrateDuoProjectLayouts(legacy: PersistedDuoLayout | undefined, projects: ProjectRecord[], selected: ProjectRecord | null) {
  if (!legacy || !selected) return {}
  const owner = (tab: PersistedDuoTab) => {
  ...
  return Object.fromEntries(projects.flatMap((project) => {
    ...
    return [[project.id, normalizeDuoLayout({ ...legacy, panes: { left: pane('left'), right: pane('right') } })!]]
  }))
}

export function scopeDuoLayout(layout: PersistedDuoLayout, project: ProjectRecord): PersistedDuoLayout {
  const pane = (side: 'left' | 'right') => ({ ...layout.panes[side], tabs: layout.panes[side].tabs.filter((tab) =>
    ...
    .map((tab) => tab.kind === 'conversation' ? { ...tab, projectId: project.id } : tab) })
  return normalizeDuoLayout({ ...layout, panes: { left: pane('left'), right: pane('right') } })!
}
