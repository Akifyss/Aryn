...
export function FileThumbnailLoadingOverlay() {
  return (
    <div
      aria-hidden="true"
      className="absolute inset-0 z-10 overflow-hidden bg-[var(--background-secondary)]"
    >
      <div className="absolute inset-0 bg-[var(--background-secondary)]" />
      <div className="absolute inset-0 animate-pulse bg-[color-mix(in_oklab,var(--background-primary)_55%,transparent)] motion-reduce:animate-none" />
    </div>
  )
}
...
export function FileThumbnail({
  ...
  return (
    <div
      className={cx(
        "group overflow-hidden rounded-lg border bg-[var(--background-primary)] text-[var(--foreground-primary)]",
        className
      )}
    >
      <div
        className={cx(
          "relative aspect-square overflow-hidden bg-[var(--background-secondary)] [contain:layout_paint]",
          previewClassName
        )}
        style={
          previewAspectRatio
      ...
      >
        ...
        {showFallback ? (
          <div className="absolute inset-0 bg-[var(--background-secondary)]" aria-hidden="true" />
        ) : null}
      </div>
    </div>
  )
