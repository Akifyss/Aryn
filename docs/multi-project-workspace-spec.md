...
- 非 active 项目的 session 文件列表优先在展开项目节点时懒加载，避免启动时遍历所有项目对应的 `~/.aryn/agents/pi/sessions/<encoded-cwd>`。
...
- `index.json` 损坏：应回退为空列表；如有必要，后续可以从 `~/.aryn/agents/pi/sessions/<encoded-cwd>/*.jsonl` 尝试重建，但本阶段不要求启动时扫描所有工作目录。
