...
- 对话行 hover 时显示与项目 session 一致的 `...` 操作入口。
- 对话操作菜单支持“重命名”和“删除”。
- 重命名只更新 `~/.aryn/conversations/index.json` 中的标题，不重命名工作目录。
- 删除只从普通对话索引和侧边栏列表中移除该对话，并释放对应运行时；不删除工作目录中的产物文件。
- 删除当前打开的普通对话后，当前上下文回到 `conversationDraft`。
...
- 归档对话。
- 删除对话时同时删除产物目录。
- 删除对话时删除 PI Agent session JSONL。
- 对话工作目录改名。
- 将普通对话转换为项目。
- 跨设备同步。
