...
Agent wrapper, its non-remounting behavior, the absence of the removed native
view toggle, and a synthetic OpenCode timeline containing conversation,
reasoning, command, web-fetch, diff, optimistic, and explicitly preserved
unknown-provider rows.
...
and 960 px surface widths, and verifies reduced-motion timing. Renderer errors,
failed requests, and shutdown-time Agent IPC errors are treated as failures
