...
describe('MEO React chrome boundary', () => {
  it('renders application chrome with shared React components and locally bundled icons', async () => {
    ...
    expect(shellSource).toContain('AppIconButton,')
    expect(shellSource).toContain("from '@/components/app-icon-button'")
    expect(shellSource).toContain('ViewerToolbarGroup,')
    expect(shellSource).toContain("from '@/components/ui/document-viewer-controls'")
    ...
    expect(shellSource).toContain("size = 'md'")
    expect(shellSource).toMatch(/MeoHeadingMenu[\s\S]*?<Menu\.Trigger[\s\S]*?size='md'/)
    expect(shellSource).toMatch(/action='findWholeWord'[\s\S]*?size='sm'/)
    ...
    expect(shellSource).toContain('<ViewerToolbar')
    expect(shellSource).toContain('<ViewerToolbarGroup>{leadingToolbarAction}</ViewerToolbarGroup>')
    expect(shellSource).not.toContain("<div\n        ref={toolbarRef}\n        className='mode-toolbar'")
  })
  ...
  it('keeps the CodeMirror engine outside React while the shell owns the outline', async () => {
    ...
    expect(hostSource).toContain('leadingToolbarAction={leadingToolbarAction}')
