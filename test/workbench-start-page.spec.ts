...
it('uses an ephemeral start tab and opens each entry in its source pane', async () => {
  const bundle = await build({
    stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
      ...
      import {WorkbenchPane} from './src/features/workbench/workbench-pane'
      ...
      import {useWorkbenchStore, createWorkbenchPane, createDefaultWorkbenchLayout} from './src/features/workbench/workbench-state'
      ...
      const noop = () => {}
      useWorkbenchStore.setState({panes: {left: {...createWorkbenchPane(), directoryOpen:false}, right: {...createWorkbenchPane(), directoryOpen:false}}})
      window.readTabs = () => ({left:useWorkbenchStore.getState().panes.left.tabs, right:useWorkbenchStore.getState().panes.right.tabs, files:useWorkspaceStore.getState().openTabs})
      function App() {
        ...
        window.selectScopedProject = id => {
          ...
          store.switchProject(project, store.projectLayouts[id] ?? createDefaultWorkbenchLayout(project))
          setProject(project)
        }
        ...
        return <div className='app-shell' style={{'--left-panel-toggle-anchor':'6px'}}>
    ...
    plugins: [{ name: 'unrelated-content-fixtures', setup(builder) {
      const fixtures: Record<string, string> = {
        ...
        'workbench-conversations': "import React,{useEffect} from 'react'; export function WorkbenchConversationView({registerCloseGuard}) {useEffect(()=>{registerCloseGuard(async()=>true)},[]);return React.createElement('input',{'aria-label':'测试草稿'})}",
      }
  ...
  try {
    ...
    await left.locator('[aria-controls="workbench-left-directory"]').click()
    await right.locator('[aria-controls="workbench-right-directory"]').click()

    for (const [pane, peer] of [[left, right], [right, left]]) {
      for (const label of ['新对话', '文件', '更改']) {
        const action = pane.locator('.workbench-start-page').getByRole('button', { name: label, exact: true })
        ...
        expect(await pane.locator('.workbench-start-page').count()).toBe(0)
        expect(await peer.locator('.workbench-start-page').count()).toBe(1)
