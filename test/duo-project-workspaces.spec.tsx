...
import { normalizeDuoProjects, type PersistedDuoLayout } from '../electron/shared/contracts/duo-layout'
import { createDefaultDuoLayout, hasOtherDuoDocumentOwner, useDuoStore } from '../src/features/duo/duo-state'
import { createDuoProjectsSnapshot } from '../src/features/duo/duo-persistence'
import { migrateDuoProjectLayouts, scopeDuoLayout } from '../src/features/duo/duo-project-layouts'
...
describe('project-owned workspaces', () => {
  it('removes closed shared documents from inactive project layouts as well', () => {
    ...
    store.switchProject(a, createDefaultDuoLayout(a))
    store.open('left', { kind: 'document', id: 'deleted' })
    store.switchProject(b, createDefaultDuoLayout(b))
    store.open('right', { kind: 'document', id: 'deleted' })
    store.reconcileDocuments(new Set())
    expect(useDuoStore.getState().projectLayouts.a.panes.left.tabs.some(tab => tab.id === 'deleted')).toBe(false)
    expect(useDuoStore.getState().panes.right.tabs.some(tab => tab.id === 'deleted')).toBe(false)
    store.open('right', { kind: 'document', id: 'deleted' })
    expect(hasOtherDuoDocumentOwner(useDuoStore.getState(), 'right', 'deleted')).toBe(false)
  })
  ...
  it('separates identical project names and retains shared file ownership and renames', () => {
    ...
    expect(hasOtherDuoDocumentOwner(useDuoStore.getState(), 'right', 'old')).toBe(true)
    ...
    expect(hasOtherDuoDocumentOwner(useDuoStore.getState(), 'left', 'new')).toBe(false)
    expect(useDuoStore.getState().panes.left.tabs.at(-1)).toEqual({ kind: 'document', id: 'new' })
  })
  ...
  it('migrates by durable ownership and Windows paths, retaining the legacy archive', () => {
    ...
    const legacy: PersistedDuoLayout = { version: 1, ratio: .6, focusedPane: 'right', panes: {
      left: { ...pane, tabs: [
    ...
    const saved = migrateDuoProjectLayouts(legacy, [projectA, projectB], projectA)
    ...
    expect(scopeDuoLayout(legacy, projectA).panes.left.tabs.map(tab => tab.id)).toEqual(['a'])
    expect(normalizeDuoProjects({ version: 1, layouts: { a: saved.a, invalid: { version: 5 } } })?.layouts).toEqual({ a: saved.a })
    expect(normalizeDuoProjects({ version: 1, layouts: [] })).toBeUndefined()
  })
})
...
function projectController(failRoot?: string) {
  ...
  const options = {
    ...
    preserveProjectTabs: true, isAgentLayout: false,
    ...
    setActiveWorkspaceContext: vi.fn(), setAgentWorkspaceState: vi.fn(), setIsAgentLayoutFixedTabActive: vi.fn(), setStatusMessage: vi.fn(),
  }
