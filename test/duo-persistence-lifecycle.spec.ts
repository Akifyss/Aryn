...
import type { PersistedDuoLayout, PersistedDuoProjects } from '../electron/shared/contracts/duo-layout'
...
it('isolates project layouts, migrates once, survives restart and rejects late restores under StrictMode', async () => {
  const bundle = await build({
    stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
      ...
      import {flushDuoPersistence} from './src/features/duo/duo-persistence'
      import {initializeRendererPersistentState} from './src/features/persistence/renderer-state'
      import {useDuoStore, DUO_FILES_ID, DUO_GIT_ID} from './src/features/duo/duo-state'
      ...
      window.inspect = () => {
        const {panes,ratio,focusedPane,initialized,project,restoring,projectLayouts} = useDuoStore.getState()
      ...
      window.flush = flushDuoPersistence
      window.customizeA = () => {
        const store = useDuoStore.getState()
        store.setDirectoryTab('left','conversation')
        store.toggleDirectory('right')
        store.open('right',{kind:'panel',id:DUO_GIT_ID})
        store.open('right',{kind:'panel',id:DUO_FILES_ID})
        store.reorder('right',DUO_FILES_ID,DUO_GIT_ID,'before')
        store.activate('right',DUO_GIT_ID)
        store.setRatio(.62)
      ...
      window.completeBackground = () => useDuoStore.getState().setProjectSession('left','draft-a',{agentId:'pi',sessionPath:'/a/new-session'})
      window.captureOpen = () => { window.target = captureDuoDocumentTarget('left') }
      window.isOpenCurrent = () => window.target.isCurrent()
      window.openFromLeft = () => {
        const target = captureDuoDocumentTarget('left')
      ...
      window.closeReference = id => useDuoStore.getState().close('left',id)
      ...
      function App() {
        ...
        const project = useDuoStore(state=>state.project)
        ...
        useDuoWorkspaceSync({isActive:true,initialized,bootstrapReady:ready,projectState:projects,conversationState:conversations,selectedProject:selected})
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), 'aryn-duo-project-lifecycle-'))
  const statePath = path.join(root, 'app-state.json')
  const initial: PersistedDuoLayout = { version: 1, ratio: .58, focusedPane: 'left', panes: {
  ...
  await new AppStateStore(statePath).update(state => ({ ...state, layout: { ...state.layout, duo: initial } }))
  ...
  try {
    for (let launch = 0; launch < 2; launch++) {
      ...
      const writes: PersistedDuoProjects[] = []
      const errors: string[] = []
      page.on('pageerror', error => errors.push(error.message))
      await page.exposeFunction('writeLayout', async (patch: { duoProjects: PersistedDuoProjects }) => {
        writes.push(patch.duoProjects)
      ...
      expect(disk.duo).toEqual(initial) // Original archive survives migration.
      expect(Object.keys(disk.duoProjects!.layouts).sort()).toEqual(['a','b','c','slow'])
