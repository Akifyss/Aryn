...
it('moves real file and Git panels with their live state, deduplicates, and routes actions to the new owner', async () => {
  ...
  const bundle = await build({ entryPoints: ['test/fixtures/duo-panel-move.tsx'], bundle: true, write: false,
    ...
    plugins: [{ name: 'unrelated-editors', setup(builder) {
      builder.onResolve({ filter: /\/(workspace-editor-content|workspace-file-preview|workspace-tree-panel|duo-conversation-list)$/ }, args => ({ path: args.path.split('/').pop()!, namespace: 'fixture' }))
  ...
  try {
    ...
    const panel = (side: string, id: string) => page.locator(`#duo-${side} .duo-panel-view:not([hidden])[data-panel-tab-id="app://fixed/${id}"]`)
    const tab = (side: string, id: string) => page.locator(`#duo-${side} .file-tab[data-tab-id="app://fixed/${id}"]`)
    ...
    expect(await page.locator('#duo-left').getByRole('tab', { name: '开始', exact: true }).isVisible()).toBe(true)
    ...
    await page.evaluate(() => document.querySelectorAll('.duo-panel-host').forEach(host => Object.defineProperty(host, 'moveBefore', { value: undefined })))
