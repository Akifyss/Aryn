...
it('moves actual editors, preserves drafts and view state, deduplicates and transfers keyboard focus', async () => {
  ...
  const bundle = await build({ entryPoints:['test/fixtures/workbench-document-move.tsx'], bundle:true, write:false,
    ...
    plugins:[{name:'fixture-assets',setup(builder){
      builder.onResolve({filter:/\/(workspace-file-system-panel|workspace-navigation-panels|workbench-conversation-list|workbench-conversations)$/},args=>({path:args.path.split('/').pop()!,namespace:'fixture'}))
  ...
  try {
    ...
    const clickMove = async (side: string) => {
      const tab = page.locator(`#workbench-${side} .file-tab.is-active`)
      await tab.hover()
      await tab.locator('.file-tab-move').click()
    }
    await page.locator('#workbench-left .cm-content').waitFor()
    await page.evaluate(() => {
    ...
    await page.locator('#workbench-right .cm-content').waitFor()
    ...
    expect(await page.locator('#workbench-left').getByRole('tab', { name: '开始', exact: true }).count()).toBe(1)
    ...
    const back = page.locator('#workbench-right .file-tab.is-active .file-tab-move')
    ...
    await page.locator('#workbench-left .monaco-editor').waitFor()
    ...
    await clickMove('left')
    await page.locator('#workbench-right .monaco-editor').waitFor()
    ...
    await page.evaluate(() => {
      ...
      document.querySelector('#workbench-left .git-diff-pierre-viewport')!.scrollTop = 400
    })
    ...
    await expect.poll(() => page.evaluate(() => document.querySelector('#workbench-right .git-diff-pierre-viewport')?.scrollTop)).toBe(400)
    expect(await page.evaluate(() => (window as any).diff().getText())).toMatch(/^diff draft /)
    expect(await page.evaluate(() => !!document.activeElement?.closest('#workbench-right'))).toBe(true)

    await page.evaluate(() => (window as any).setup('history'))
    await page.locator('#workbench-left diffs-container').waitFor()
    await expect.poll(() => page.evaluate(() => {
      const viewport = document.querySelector('#workbench-left .git-diff-pierre-viewport')!
