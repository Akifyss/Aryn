...
describe('Workbench directory controls', () => {
  it('adds the conversation segment only when the workbench supplies it', () => {
    ...
    const workbench = renderToStaticMarkup(<WorkspaceSidebarTabs activeTab='conversation' filePanel='files' gitPanel='changes' conversationPanel={<span>shared conversation tree</span>} onActiveTabChange={() => {}} />)
    expect(workbench).toContain('对话')
    expect(workbench).toContain('shared conversation tree')
    expect(workbench.match(/role="tab"/g)).toHaveLength(3)
  })

  it.each(['left', 'right'] as const)('connects the %s toggle to its own sidebar', (side) => {
    const id = `workbench-${side}-directory`
