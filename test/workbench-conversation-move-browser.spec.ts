...
it('moves the real conversation provider and composer without interrupting drafts, creation or streaming', async () => {
  const mocks: Record<string, string> = {
    ...
    'agent-sidebar': `import React from 'react';
      ...
      import {Probe} from './test/fixtures/workbench-conversation-probe';
  ...
  const bundle = await build({ entryPoints: ['test/fixtures/workbench-conversation-move.tsx'], bundle: true, write: false,
    ...
    plugins: [{ name: 'unrelated-surfaces-and-provider-probe', setup(builder) {
      builder.onResolve({ filter: /\/(workspace-editor-content|workspace-file-system-panel|workspace-navigation-panels|workbench-conversation-list|bb-session-timeline|bb-session-surface-loader|agent-sidebar)$/ }, args => ({ path: args.path.split('/').pop()!, namespace: 'fixture' }))
  ...
  try {
    ...
    const editor = (side: string) => page.locator(`#workbench-${side} .workbench-conversation-view:not([hidden]) .agent-composer-editor`)
    const model = (side: string) => page.locator(`#workbench-${side} .workbench-conversation-view:not([hidden]) .agent-model-cascader-trigger`)
    const move = (side: string) => page.locator(`#workbench-${side} .file-tab.is-active .file-tab-move`)
    const clickMove = async (side: string) => {
      await page.locator(`#workbench-${side} .file-tab.is-active`).hover()
    ...
    await page.locator('#workbench-left .file-tab.is-active').hover()
    ...
    expect(await page.locator('#workbench-right').getByText('notes.txt', { exact: true }).isVisible()).toBe(true)
    expect((await layout()).panes.left.tabs).toEqual([])
    await page.locator('#workbench-left').getByRole('tab', { name: '开始', exact: true }).waitFor()
    ...
    await page.evaluate(() => {
      for (const host of document.querySelectorAll('.workbench-conversation-host')) Object.defineProperty(host, 'moveBefore', { value: undefined })
    ...
    await page.locator('#workbench-right button[type=submit]').click()
