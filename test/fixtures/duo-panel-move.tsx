...
import { DuoPane, type DuoPaneCommands, type DuoPaneConfiguration } from '../../src/features/duo/duo-pane'
import { DuoPanelLayer } from '../../src/features/duo/duo-panel-layer'
import { createDuoPane, DUO_FILES_ID, DUO_GIT_ID, useDuoStore, type DuoPaneId } from '../../src/features/duo/duo-state'
...
const emptyLayout = () => ({ ratio: .5, focusedPane: 'left' as const, panes: {
  left: { ...createDuoPane(), directoryOpen: false }, right: { ...createDuoPane(), directoryOpen: false },
} })
useDuoStore.getState().switchProject(project('qa'), emptyLayout())
useWorkspaceStore.setState({ currentPath: '/qa', openTabs: [], activeTabId: null })
useDuoStore.getState().open('left', { id: DUO_FILES_ID, kind: 'panel' })
useDuoStore.getState().open('right', { id: DUO_GIT_ID, kind: 'panel' })
useDuoStore.getState().open('right', { id: DUO_FILES_ID, kind: 'panel' })
test.inspect = () => ({ ...useDuoStore.getState(), documents: useWorkspaceStore.getState().openTabs })
test.open = (pane: DuoPaneId, id: string) => useDuoStore.getState().open(pane, { id: id === 'files' ? DUO_FILES_ID : DUO_GIT_ID, kind: 'panel' })
test.activate = (pane: DuoPaneId, id: string) => useDuoStore.getState().activate(pane, id === 'files' ? DUO_FILES_ID : DUO_GIT_ID)
...
function App() {
  const selectedProject = useDuoStore(state => state.project!)
  const root = selectedProject.path
  const commands = useRef<Partial<Record<DuoPaneId, DuoPaneCommands>>>({})
  ...
  const configuration = {
    editor: {
      ...
      fileTabs: { iconTheme: null, workspacePath: root }, emptyState: {},
      fileSystemPanel: { nodes, workspacePath: root, title: selectedProject.name, iconTheme: null, theme: 'light',
  ...
  return <div className='app-shell duo-shell' data-app-layout='duo' style={{ '--left-panel-toggle-anchor': '6px', '--duo-left-controls-width': '0px' } as React.CSSProperties}>
...
void loadDuoLayout(createDuoLayoutSnapshot(useDuoStore.getState(), [], '/qa'), app.appApi, [project('qa')], []).then(restored => {
  useDuoStore.setState({ panes: restored.panes })
