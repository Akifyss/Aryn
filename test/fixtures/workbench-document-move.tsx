...
import { WorkbenchPane, type WorkbenchPaneConfiguration, type WorkbenchPaneCommands } from '../../src/features/workbench/workbench-pane'
...
app.setup = (kind = 'meo', duplicate = false) => {
  ...
  useWorkbenchStore.setState({ ...useWorkbenchStore.getInitialState(), initialized: true,
    panes: { left: { ...createWorkbenchPane(), directoryOpen: false }, right: { ...createWorkbenchPane(), directoryOpen: false } } })
...
app.staleRequests = () => {
  app.lateLeft = captureWorkbenchDocumentTarget('left')
  app.lateRight = captureWorkbenchDocumentTarget('right')
}
app.cm = (side: string) => {
  const element = Array.from(document.querySelectorAll<HTMLElement>(`#workbench-${side} .cm-editor`))
...
app.code = (side: string) => monaco.editor.getEditors().find(editor => editor.getDomNode()?.closest(`#workbench-${side}`))
...
function App() {
  ...
  const configuration = {
    editor: {
      ...
      fileTabs: { iconTheme: null, workspacePath: '/qa' }, fileSystemPanel: {}, }, conversations: {}, documentNavigation: {}, refreshGitState: noop,
