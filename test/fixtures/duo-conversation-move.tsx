...
import { createDefaultDuoLayout, openDuoProjectSession, useDuoStore } from '../../src/features/duo/duo-state'
...
const layout = createDefaultDuoLayout(project)
layout.panes.left.directoryOpen = false
useDuoStore.setState({ ...useDuoStore.getInitialState(), ...layout, project, initialized: true })
useWorkspaceStore.setState({ currentPath: project.path, openTabs: [], activeTabId: null })
app.draftId = layout.panes.left.activeTabId
app.inspect = () => useDuoStore.getState()
app.context = (side: string) => {
  const node = document.querySelector(`#duo-${side} .duo-conversation-view:not([hidden]) [data-probe-id]`) as HTMLElement
...
app.openNew = (side: 'left' | 'right') => openDuoProjectSession(side, project)
app.reopen = (side: 'left' | 'right') => openDuoProjectSession(side, project, { agentId: 'pi', sessionPath: 'created', sessionLabel: 'Created' })
app.activate = (side: 'left' | 'right', id: string) => useDuoStore.getState().activate(side, id)
...
function App() {
  ...
  const configuration = {
    editor: {
      ...
      fileTabs: { iconTheme: null, workspacePath: project.path }, fileSystemPanel: {}, emptyState: {},
    },
