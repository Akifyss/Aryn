...
it('keeps both committed panes visible until project restoration commits, including late results and errors', async () => {
  const bundle = await build({
    stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
      ...
      import {WorkspaceShell} from './src/features/workbench/workspace-shell'
      import {useProjectWorkspaceSync} from './src/features/workbench/use-project-workspace-sync'
      import {useWorkbenchStore} from './src/features/workbench/workbench-state'
      ...
      initializeRendererPersistentState({app:{layout:{projectWorkspaces:{version:1,layouts:{b:saved('b'),slow:saved('slow')}}}},workspace:{workspaceTabs:{}}})
      const gates={}
      ...
      window.inspect=()=>({project:useWorkbenchStore.getState().project?.id,restoring:useWorkbenchStore.getState().restoring,documents:useWorkspaceStore.getState().openTabs.map(tab=>tab.filePath)})
      window.capture=()=>{window.oldTarget=captureWorkbenchDocumentTarget('left')}
      const noop=()=>{}
      function App(){
        ...
        useProjectWorkspaceSync({initialized,bootstrapReady:true,projectState,conversationState,selectedProject:selected,workspaceUnavailableMessage:unavailable})
        const configuration={editor:{
          ...
          fileTabs:{iconTheme:null,workspacePath:cwd},fileSystemPanel:{workspacePath:cwd,title:cwd},},conversations:{selectedProject:selected,projectState},documentNavigation:{},refreshGitState:noop,onCloseDocument:async()=>true,confirmCloseConversation:async()=>true}
        return <WorkspaceShell ref={shellRef} configuration={configuration} chromeVars={getShellChromeVars('windows')} platform='windows' isFullScreen={false}
    ...
    plugins: [{ name: 'content-fixtures', setup(builder) {
      const fixtures: Record<string, string> = {
        ...
        'workbench-conversation-list': `import React from 'react';export const WorkbenchConversationList=({configuration})=><div>{configuration.selectedProject.name} sessions</div>`,
        'workbench-conversations': `import React from 'react';export function WorkbenchConversationView({tab,registerCloseGuard}){registerCloseGuard(async()=>true);return <input aria-label={'Draft '+tab.projectSession.project.id}/>}`,
      }
      builder.onResolve({ filter: /\/(workspace-editor-content|workspace-file-system-panel|workspace-navigation-panels|workbench-conversation-list|workbench-conversations)$/ }, args => ({path:args.path.split('/').pop()!,namespace:'fixture'}))
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      app.frames = []
      const sample = () => {
        const main = document.querySelector('.workbench-panes')!
        app.frames.push({visibility:getComputedStyle(main).visibility,
          title:document.querySelector('.workbench-workspace-switch-label')?.textContent,
          files:document.querySelector('.workbench-panel-view:not([hidden]) .fixture-files')?.textContent,
          draft:document.querySelector('.workbench-conversation-view:not([hidden]) input')?.getAttribute('aria-label')})
    ...
    const assertAVisible = async () => {
      expect(await page.locator('.workbench-panes').evaluate(el=>getComputedStyle(el).visibility)).toBe('visible')
      expect(await page.locator('.workbench-panes').getAttribute('inert')).not.toBeNull()
      expect(await page.locator('.workbench-workspace-switch-label').innerText()).toBe('a')
      expect(await page.locator('.fixture-files:visible').innerText()).toBe('/a files')
      expect(await page.locator('.workbench-conversation-view:not([hidden]) input').inputValue()).toBe('keep my draft')
    ...
    await expect.poll(()=>page.locator('.workbench-panes').getAttribute('inert')).toBeNull()
    ...
    expect(await page.locator('.workbench-workspace-switch-label').innerText()).toBe('b')
