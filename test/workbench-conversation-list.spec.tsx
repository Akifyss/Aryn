...
beforeEach(() => {
  ...
  useWorkbenchStore.setState({ panes: { left: createWorkbenchPane(), right: createWorkbenchPane() }, focusedPane: 'right' })
})

describe('Workbench shared project and conversation navigation', () => {
  it('uses the shared project session browser, excluding other projects and standalone data', () => {
    ...
    const markup = renderToStaticMarkup(<WorkbenchConversationList pane='left' configuration={configuration} />)
    expect(markup).toContain('data-shared-agent-tree')
    expect(markup).not.toContain('workbench-conversation-items')
  ...
  it.each(['left', 'right'] as const)('opens project history in the %s pane without changing its peer', async (pane) => {
    renderToStaticMarkup(<WorkbenchConversationList pane={pane} configuration={{ selectedProject: project }} />)
    const provider = captured.provider!
    await provider.onOpenProjectSession!(project, 'pi', '/workspace/session.jsonl', '历史会话')
    const firstId = useWorkbenchStore.getState().panes[pane].activeTabId
    await provider.onOpenProjectSession!(project, 'pi', '/workspace/session.jsonl', '历史会话')
    expect(useWorkbenchStore.getState().panes[pane].activeTabId).toBe(firstId)
    expect(useWorkbenchStore.getState().panes[pane].tabs).toHaveLength(1)
    expect(useWorkbenchStore.getState().panes[pane].tabs[0]).toMatchObject({ projectSession: { project, request: { kind: 'session', agentId: 'pi', sessionPath: '/workspace/session.jsonl' } } })
  ...
  it('requires a selected project instead of loading a standalone runtime', () => {
    const markup = renderToStaticMarkup(<WorkbenchConversationList pane='left' configuration={{ selectedProject: null }} />)
  ...
  it.each(['left', 'right'] as const)('the %s list direction action moves an already open session to its peer', async pane => {
    ...
    const tab = useWorkbenchStore.getState().panes[pane].tabs[0]
    ...
    expect(useWorkbenchStore.getState().panes[peer].tabs).toEqual([tab])
    expect(useWorkbenchStore.getState().panes[peer].tabs[0]).toBe(tab)
    expect(useWorkbenchStore.getState().focusedPane).toBe(peer)
  })
})
