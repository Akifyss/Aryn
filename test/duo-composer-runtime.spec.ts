...
// Exercise the real Duo hosts, AgentProvider, navigation/lifecycle hooks and
  ...
  const bundle = await build({
    stdin: {
      resolveDir: process.cwd(), loader: 'tsx', contents: `
        ...
        import { DuoConversationView } from './src/features/duo/duo-conversations'
        ...
        function Fixture(){
          ...
          return <>
            ...
            <main><DuoConversationView pane='left' tab={tab} configuration={chatConfiguration} onOpenFile={()=>{}} registerCloseGuard={()=>{}} confirmClose={async()=>true}/></main>
            {directories?<aside><DuoConversationList pane='left' configuration={configuration}/><DuoConversationList pane='right' configuration={configuration}/></aside>:null}
