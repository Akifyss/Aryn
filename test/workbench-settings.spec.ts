...
beforeAll(async () => {
  const bundle = await build({
    stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
      ...
      function App() {
        const overlay = useAppOverlayController({ hasConfirmation: false,
        ...
        return <>
          <WorkspaceShell configuration={{conversations:{}}} chromeVars={{}} platform='windows'
            isFullScreen={false} isModalOpen={overlay.isAppModalLayerOpen}
    ...
    outfile: 'workbench-settings-fixture.js',
    plugins: [{ name: 'unrelated-pane-content', setup(builder) {
      ...
      // Exercise real settings buttons in the workspace, the controller and entire dialog.
      builder.onResolve({ filter: /\/(workbench-pane|workbench-panel-layer|workbench-conversation-layer|app-titlebar)$/ },
        (args) => ({ path: args.path.split('/').pop()!, namespace: 'fixture' }))
      builder.onLoad({ filter: /.*/, namespace: 'fixture' }, (args) => ({
        contents: args.path === 'workbench-pane'
          ? 'export const WorkbenchPane = () => null'
          : args.path === 'workbench-panel-layer' ? 'export const WorkbenchPanelLayer = () => null'
          : args.path === 'workbench-conversation-layer' ? 'export const WorkbenchConversationLayer = () => null'
...
it.each(['mouse', 'keyboard'])(
  'opens and remembers workspace settings via %s without treating the event as a section', async (input) => {
  const page = await browser.newPage({ viewport: { width: 1600, height: 900 } })
  page.setDefaultTimeout(4000)
  const errors: string[] = []
  page.on('pageerror', (error) => errors.push(error.message))
