...
describe('agent session loading state', () => {
  it('commits the target chrome immediately and keeps runtime loading out of the message gate', async () => {
    ...
    expect(surfaceSource).not.toContain('正在创建会话…')
    expect(surfaceSource).toMatch(/const isNewConversation = isConversationMaterializing\s*\|\| shouldShowAgentNewConversationPrompt/)
    expect(surfaceSource).toMatch(/\{!isConversationMaterializing && showSessionLoadingIndicator \? \([\s\S]*?<AppLoadingState[\s\S]*?\) : isNewConversation \? \(/)
    expect(surfaceSource).toContain('sessionStatus={sessionStatus}')
    ...
    expect(sidebarSource).toMatch(/const hasVisibleNativeSession = Boolean\([\s\S]*?codexNativeSession[\s\S]*?openCodeNativeSession[\s\S]*?piWebNativeSession/)
    expect(sidebarSource).toMatch(/const isConversationMaterializing = shouldRetainNewConversationSurfaceDuringSubmission\(\{[\s\S]*?conversationStatus: activeConversation\?\.status \?\? null,[\s\S]*?hasVisibleNativeSession,[\s\S]*?isSubmitting: isSubmittingComposerPrompt/)
    expect(sidebarSource).toContain('optimisticUserMessages: codexOptimisticUserMessages,')
    expect(sidebarSource).not.toContain('visiblePendingConversationSubmission')
  ...
  it('presents new-conversation drafts immediately while runtime-dependent actions stay gated', async () => {
    ...
    expect(sidebarSource).toMatch(/\(canUseVisibleWorkspace && agentState\.runtime\.hasConfiguredModels\)/)
    expect(sidebarSource).toMatch(/const canPresentComposerCapabilities = !isWorkspaceContextPreparing[\s\S]*?&& \(canUseVisibleWorkspace \|\| canUseComposerWithoutWorkspace\)/)
    expect(sidebarSource).toMatch(/const attachmentCapabilityMessage = canPresentComposerCapabilities/)
    expect(sidebarSource).toMatch(/const statusMessage = isWorkspaceContextPreparing\s*\? null/)
    expect(sidebarSource).toContain("!sessionPresentation.workspacePath && activeWorkspaceContext.kind === 'conversation'")
    ...
    expect(composerSource).toMatch(/resolveAgentComposerWorkspacePresentation\(\{[\s\S]*?isWorkspaceContextPreparing,[\s\S]*?operationalWorkspacePath: workspacePath,[\s\S]*?visibleWorkspacePath,/)
    expect(composerSource).toContain('!canUseOperationalWorkspace && !canUseComposerWithoutWorkspace')
    expect(composerSource).toMatch(/onFilesPastedOrDropped=\{isWorkspaceContextPreparing[\s\S]*?\|\| \(!canUseOperationalWorkspace && !canUseComposerWithoutWorkspace\)/)
    expect(composerSource).toContain('placeholder={composerPlaceholder}')
    expect(composerSource).toContain('workspacePath={mentionWorkspacePath}')
