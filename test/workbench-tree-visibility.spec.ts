...
beforeAll(async () => {
  const bundle = await build({
    entryPoints: ['test/fixtures/workbench-tree-visibility.tsx'],
    bundle: true, write: false, platform: 'browser', format: 'iife',
    define: { 'process.env.NODE_ENV': '"development"' }, alias: { '@': path.resolve('src') },
    outfile: 'workbench-tree-fixture.js',
    plugins: [{ name: 'unrelated-content-fixtures', setup(builder) {
      // Keep the actual Workbench lists, navigation, rows, virtualizers and scroll areas.
      const fixtures: Record<string, string> = {
        ...
        'workbench-conversations': 'export const WorkbenchConversationView = () => null',
      }
      builder.onResolve({ filter: /\/agent-sidebar\/agent-sidebar$/ },
        () => ({ path: path.resolve('test/fixtures/workbench-sidebar-agent-provider.tsx') }))
      builder.onResolve({ filter: /\/(workspace-editor-content|workspace-file-system-panel|workbench-conversations)$/ },
...
async function mountSidebarFixture(page: Page, gitLayout = 'list') {
  ...
  await page.locator('#workbench-left').waitFor()
  await settleMeasurements(page)
}
...
async function captureSidebar(page: Page, side: string, name: string) {
  const directory = process.env.ARYN_WORKBENCH_QA_SCREENSHOTS
  if (!directory) return
  await mkdir(directory, { recursive: true })
  await page.locator(`#workbench-${side} .editor-directory-sidebar`).screenshot({ path: path.join(directory, `${name}-${side}.png`) })
}
...
it.each(['left', 'right'] as const)('opens conversation rows locally and across panes from the %s list', async (side) => {
  ...
  try {
    ...
    const sidebar = page.locator(`#workbench-${side} .editor-directory-sidebar`)
...
it.each(['left', 'right'] as const)('opens a row locally and its arrow in the opposite pane from the %s tree', async (side) => {
  ...
  try {
    ...
    const tree = page.locator(`#workbench-${side} .workspace-tree-root`)
    ...
    await arrow.click()
    await expect.poll(() => page.locator(`#workbench-${other} .file-tab-label`).allTextContents()).toContain('file-0.md')
    expect(await page.locator(`#workbench-${side} .file-tab-label`).allTextContents()).toEqual(['开始'])
    ...
    await expect.poll(() => page.locator(`#workbench-${side} .file-tab-label`).allTextContents()).toContain('file-0.md')
    ...
    await page.keyboard.press('Enter')
    await expect.poll(() => page.locator(`#workbench-${other} .file-tab-label`).allTextContents()).toContain('file-1.md')
    expect(await page.locator(`#workbench-${side} .file-tab-label`).allTextContents()).toEqual(['file-0.md'])
    await row('file-0.md').hover()
...
it.each([
...
])('restores both Workbench trees after hiding ($initiallyHidden, files=$fileCount, measured=$tallRows)', async (options) => {
  const page = await browser.newPage({ viewport: { width: 1760, height: 835 } })
  page.setDefaultTimeout(4000)
  const errors: string[] = []
  page.on('pageerror', (error) => errors.push(error.message))
  try {
    ...
    await page.addScriptTag({ content: script })
    await page.locator('#workbench-left').waitFor({ state: 'attached' })
    await settleMeasurements(page)
    if (options.initiallyHidden) await setVisible(page, true)

    const trees = ['left', 'right'].map((side) => page.locator(`#workbench-${side} .workspace-tree-root`))
    ...
    const viewports = ['left', 'right'].map((side) => page.locator(`#workbench-${side} .workspace-tree-scroll > .app-scroll-area-viewport`))
    if (options.tallRows) {
